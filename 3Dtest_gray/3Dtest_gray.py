...
syspath.insert(0, "/Games/3Dtest_gray")
...
thumbyGrayscale.display.setFPS(50)

screen = bytearray(72*40)
dithermask = bytearray(open("/Games/3Dtest_gray/mask.bin", "rb").read())
dithermask_gs = bytearray(open("/Games/3Dtest_gray/mask_gs.bin", "rb").read())
...
polygons = [
[-136, 136, -225, -136, 136, 225, 160, 0, 64], #face
[-75, 75, -167, -75, 75, -56, 50, 0, 32], #screen
[-78, -40, 12, -78, -40, 112, 50, 0, 32], #dpad vertical
[-109, -9, 43, -109, -9, 80, 50, 0, 32], #dpad horizontal
[37, 56, 51, 25, 67, 63, 50, 0, 32], #B top
...
[71, 113, 33, 71, 113, 52, 50, 0, 32], #A middle
[71, 113, 52, 83, 102, 63, 50, 0, 32], #A bottom
[-136, 136, 225, -136, 136, -225, 160, math.pi, 64], #back
]
...
@micropython.viper
def ditherscreen_gray_basic(offset:int):
    display = ptr8(thumbyGrayscale.display.buffer)
    shade = ptr8(thumbyGrayscale.display.shading)
    scr = ptr8(screen)
    index:int = 0
    ...
    for y in range(40):
        ...
        bufpos = (y >> 3) * 72
        for x in range(72):
            if scr[index] >= 128: #is bright
                display[bufpos + x] |= bit
            if 64 <= scr[index] < 192: #is a middle shade
                shade[bufpos + x] |= bit
            index += 1
...
while True: #all of this is temporary for testing
    ...
    for poly in range(5): #outer faces
        ...
        if not top[2] > bot[2]: #outer face visible
            shade = int(math.sin(rot+math.pi/2 + math.pi*2/5*poly + math.pi)*80) + 150
            fquad(bot[0], bot[1], bot[2], top[0], top[1], top[2], shade)
    
    #fquad(0, 72, 0, 0, 72, 40, min(255, max(0, int(gpos[0]*4))))
    #ditherscreen(0)
    #ditherscreen_white(0)
    #ditherscreen_gray_basic(0)
    ditherscreen_gray(0)
    ...
    if time.ticks_ms() > nexttime:
        ...
        frames = 0
    #gc.collect()
    if thumby.buttonA.pressed() and thumby.buttonB.pressed(): break
